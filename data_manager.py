...
def get_image_by_question_id_dm(question_id):
    data_questions = connection.read_dict_from_file(questions_csv)
    image =next((line["image"] for line in data_questions if line['id'] == question_id),"-")
    return image
...
def get_answers_by_question_id_dm(question_id):
    ...
    answers = [[line["submission_time"],line['message'], line["id"], line["image"]] for line in sorted_answers if line['question_id'] == question_id]
    return answers
